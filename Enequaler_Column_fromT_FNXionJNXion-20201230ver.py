#Enequaler_Column_fromT_FNXionJNXion-20201230ver.py
...
print("PLZ choose your XLSX file, and press Enter-key, e.g. fnxion_dequaled_wizT (without [.xlsx]).")
XLSX0 = input()
XLSX1 = XLSX0 + '.xlsx'
# ワークブック(例:「fnxion_dequaled_wizT.xlsx」)を開く(Excelファイルを読み込む)。
...
for cols1 in sheet1.iter_cols(min_row=fromminRow, min_col=fromminColumn, max_col=toMaxColumn):
    for cell1 in cols1:
...
# 念の為、別名で保存しておく
book1.save(XLSX0 + '_JNXion_fromT_EnequaledCol_FNXion.xlsx')
 
    
# 読み込んでいたExcelファイルを閉じる
book1.close()

# “FNXionJNXion (Enequaler_Column) in Python 3.8” produced by K.masamix “KIXAN” as the SHYNAMITES/La CHENAMITOJ, 2020/12/30.
