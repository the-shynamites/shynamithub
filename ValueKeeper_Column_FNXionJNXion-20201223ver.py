#ValueKeeper_Column_FNXionJNXion-20201223ver.py
...
#値の入った列の範囲をR1C1表記で指定する(例:D1〜D10までのセルを指定する)。
...
for cols4 in sheet1.iter_cols(min_row=fromNEWminRow, min_col=fromNEWminColumn,  max_col=toNEWMaxColumn):
    for cell4 in cols4:
    	cell4.value = v
...
# 念の為、別名で保存しておく
book1.save(XLSX0 + '_ValueKeptColumn_FNXion_JNXion.xlsx')
 
    
# 読み込んでいたExcelファイルを閉じる
book1.close()

# “FNXionJNXion (ValueKeeper_Column) in Python 3.8” produced by K.masamix “KIXAN” as the SHYNAMITES/La CHENAMITOJ, 2020/12/23.
